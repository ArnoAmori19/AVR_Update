ALTER PROCEDURE [dbo].[t_SaleCRCheque](@ChID BIGINT, @HardWareDiscount BIT = 0) 
...
BEGIN 
  ...
  */ 
  ...
  IF @HardWareDiscount = 1 
    BEGIN 
      ...
      SELECT MIN(d.SrcPosID) SrcPosID, d.ProdID, p.UM, (CASE WHEN @TaxPayer = 1 THEN d.TaxTypeID ELSE 1 END) TaxTypeID, 
		  ...
		  (CASE WHEN p.ExciseGrID in (1,2,3) THEN CstProdCode ELSE '' END) CstProdCode /*Код Уктзед*/
        ...
        , d.RealBarCode, g.GroupField, g.IsBonus, CstProdCode, p.ExciseGrID
  ...
  SELECT MIN(d.SrcPosID) SrcPosID, d.ProdID,  p.UM, (CASE WHEN @TaxPayer = 1 THEN d.TaxTypeID ELSE 1 END) TaxTypeID
  , d.PriceCC_wt TPriceCC_wt, d.PurPriceCC_wt TPurPriceCC_wt,SUM(d.Qty) TQty, (CASE @UseProdNotes WHEN 0 THEN p.ProdName ELSE p.Notes END) ProdName
  , (CASE WHEN p.ExciseGrID in (1,2,3) THEN CstProdCode ELSE '' END) CstProdCode /*Код Уктзед*/
  FROM t_SaleTempD d WITH(NOLOCK)
  JOIN r_Prods p WITH(NOLOCK) ON d.ProdID = p.ProdID
  JOIN @GroupSrcPosID_Table g ON d.SrcPosID = g.SrcPosID
  ...
  , d.RealBarCode, g.GroupField, g.IsBonus, p.Article3, p.CstProdCode, p.ExciseGrID
END
